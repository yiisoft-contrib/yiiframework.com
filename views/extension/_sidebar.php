...
<ul class="extension-side-menu">
    ...
    <?php foreach(ExtensionCategory::findWithCountData()->all() as $cat): ?>
        <li<?= isset($category) && $category == $cat->id ? ' class="active"' : '' ?>>
            <a href="<?= \yii\helpers\Url::to(['extension/index', 'category' => $cat->id, 'tag' => isset($tag) ? $tag->slug : null])?>"><?= Html::encode($cat->name) ?> <span class="count">(<?= (int) $cat->count ?>)</span></a>
        </li>
    <?php endforeach; ?>
</ul>
...
<ul class="extension-side-menu last-side-menu">
    <?php foreach(ExtensionTag::find()->orderBy(['frequency' => SORT_DESC])->limit(10)->all() as $t): ?>
        <li<?= isset($tag) && $tag->equals($t) ? ' class="active"' : '' ?>>
            <a href="<?= \yii\helpers\Url::to(['extension/index', 'tag' => $t->slug, 'category' => isset($category) ? $category : null])?>"><?= Html::encode($t->name) ?> <span class="count">(<?= (int) $t->frequency ?>)</span></a>
        </li>
    <?php endforeach; ?>
</ul>
