...
yii-1.0: composer
	...
	COMPOSER=composer.yii-1.0.json php data/composer.phar install --no-cache

yii-1.1: composer
	test -d data/yii-1.1 || git clone https://github.com/yiisoft/yii.git data/yii-1.1
	cd data/yii-1.1 && git pull
	cd data/yii-1.1 && php ../composer.phar require --dev --prefer-dist --no-cache "phpunit/phpunit:4.8.34" "phpunit/phpunit-selenium:~1.4.0"
